...
const zhiyeList = [{
    ...
    icon: 'icon-bumen',
    content: '寺库-某某事业群-某某 平台-某某设计部-UI'
}, {
    icon: 'icon-dizhi',
    content: '北京市朝阳区'
}];
                ...
                <Row justify="space-between">
                    <Col span="9">
                        <div className="info-left-touxiang">
                            <Result imgUrl="../resouce/user.png" text="Thomas Hall" resultTxt="这个世界需要你" showBtn={false}></Result>
                        </div>
                            ...
                            <NavTab tabPosition="top" defaultActiveKey={0} disabledKey={1}>
                                {
                                    navs.map((item,key)=>{
                                        return (
                                            ...
                                            <NavContents activeKey={this.state.activeKey} tab={item.name + '('+item.number+')'} key={item.index}>
                                                {
                                                    [1,2,3,4].map((item)=>{
                                                        return (
                                                            <div className="cur-content-list" key={item}>
                                                                ...
                                                                <div className="text">
                                                                    啥的老公撒娇了的广东省管理第三个拉三等奖格拉斯加告诉大家更垃圾啊噶个 ，三大感觉拉杆夹，爱上大家来噶十多个家连锁店，谁打过来尬鸡蛋干，阿萨德古 拉结果拉十多个的撒狗粮收到几个啦记录sad沈杜公路sad结果拉三季度感觉。dsag，熬过来
                                                                </div>
                                                                <div className="time">
                                                                    <img src={require('../../../resouce/user.png')} alt=""/>
                                                                    <span className="blue-txt"> 陆秀英 </span>
                                                                    <span>发布在 </span>
